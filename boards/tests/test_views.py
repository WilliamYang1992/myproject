...
class HomeTests(TestCase):
    ...
    def test_home_view_contains_link_to_topics_page(self):
        board_topics_url = reverse('board-topics', kwargs={'pk': self.board.pk})
...
class BoardTopicsTests(TestCase):
    ...
    def test_board_topics_view_success_status_code(self):
        url = reverse('board-topics', kwargs={'pk': 1})
        response = self.client.get(url)
        self.assertEquals(response.status_code, 200)

    def test_board_topics_view_nod_found_status_code(self):
        url = reverse('board-topics', kwargs={'pk': 999})
        response = self.client.get(url)
    ...
    def test_board_topics_view_contains_navigation_links(self):
        board_topics_url = reverse('board-topics', kwargs={'pk': 1})
...
class NewTopicTests(TestCase):
    def setUp(self):
        Board.objects.create(name='Django', description='Django board.')
        User.objects.create_user(username='John', email='John@example.com', password='123456')
    ...
    def test_new_topic_view_not_found_status_code(self):
        url = reverse('new-topic', kwargs={'pk': 999})
        response = self.client.get(url)
        self.assertEquals(response.status_code, 404)
